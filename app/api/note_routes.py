...
@login_required
def notes():
    all_notes = Notes.query.filter(Note.user_id == current_user.id).all()
...
def delete_note(note_id):
    ...
    if note:
        ...
        db.session.commit()
        return jsonify({'message': f'Note with id of {note_id} deleted successfuly.'}), 200
